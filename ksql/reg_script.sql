...
SELECT 
  PARSE_DATE(CONCAT_WS('-', SUBSTRING(call_start_date,0,4),SUBSTRING(call_start_date,5,2),SUBSTRING(call_start_date,7,2)) ,'yyyy-MM-dd') as start_date,
  SUBSTRING(call_start_date,7,2) as day,
...
SELECT a.AFTER->INDEX as INDEXa,b.INDEX as INDEXb,a.AFTER->E as Groups,a.AFTER->O as Missed_Call,
...
INNER JOIN DATE_TIME_COLUMN  WITHIN 3 second on a.AFTER->INDEX=DATE_TIME_COLUMN.INDEX where a.AFTER->AF is not null and DATE_TIME_COLUMN.START_DATE is not null
EMIT CHANGES;


DROP STREAM if exists service_datasets;
SET 'auto.offset.reset' = 'earliest';
CREATE STREAM service_datasets WITH (VALUE_FORMAT = 'JSON') AS
...
where a.AFTER->AF is not null and DATE_TIME_COLUMN.START_DATE is not null
EMIT CHANGES;
